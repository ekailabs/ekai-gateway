...
export class XAIResponsesPassthrough implements ResponsesPassthrough {
  ...
  private get apiKey(): string {
    ...
    const fallback = getConfig().providers.xai.apiKey;
    if (fallback) return fallback;
    throw new AuthenticationError('xAI API key not configured', { provider: this.config.provider });
  }
  ...
  private usage: { inputTokens?: number; outputTokens?: number; totalTokens?: number } | null = null;
  private eventBuffer = '';
  ...
  private async makeRequest(body: any, stream: boolean): Promise<globalThis.Response> {
    ...
    if (!response.ok) {
      ...
      throw new ProviderError('xai', errorText || `HTTP ${response.status}`, response.status, { endpoint: this.baseUrl });
    }
    return response;
  }
        ...
        try {
          ...
          if (data.response?.usage) {
            ...
            import('../utils/usage-tracker.js').then(({ usageTracker }) => {
              usageTracker.trackUsage(model, 'xai', nonCachedInputTokens, outputTokens, cachedTokens, 0, clientIp);
            }).catch(() => {});
          }
        } catch {}
        this.eventBuffer = '';
  ...
  async handleDirectRequest(request: any, res: ExpressResponse, clientIp?: string): Promise<void> {
    ...
    if (request.stream) {
      ...
      persistMemory(request, this.assistantResponseBuffer, {
        ...
        metadataBuilder: req => ({ model: req.model, provider: this.config.provider }),
      });
    } else {
